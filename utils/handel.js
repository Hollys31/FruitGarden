...
//https://trace.yufengtek.com/orchard/applet/api
      ...
      success(Local) {
        ...
        qqmapsdk.reverseGeocoder({
          ...
          success: function (LocalRes) {
            ...
            wx.setStorage({ key: 'orisechProv', data: provice })
            wx.setStorage({ key: 'orisechCity', data: city })
            wx.setStorage({ key: 'orisechDistrict', data: district })
            wx.setStorage({ key: 'orisechAddr', data: address })
            wx.hideLoading()
          },
          fail: function (error) {
            wx.showToast({
  ...
  checkCode(that, data) {
    ...
    _this.handelRequest(that, {
      ...
      console.log(res);
      wx.setStorage({ key: 'qrcodeNum', data: res.qrcodeNum });
      wx.setStorage({ key: 'blockId', data: res.gardenInfo.blockId })
      wx.setStorage({ key: 'gardenId', data: res.gardenInfo.gardenId })
      wx.reLaunch({
  ...
  getPhoneNumber(e,that,code){//获取用户手机号
    ...
    _this.handelRequest(that, {
      ...
      if (code) {
        ...
        _this.handelScanCode(that, { phoneNumber: result.phoneNumber, openId: wx.getStorageSync('openId') || '' }, code);
      } else {
        _this.handelScanCode(that, { phoneNumber: result.phoneNumber, openId: wx.getStorageSync('openId') || '' });
      }
    })
  },
